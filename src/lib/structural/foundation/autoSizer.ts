...
// ============================================================================
// 2. PUNCHING SHEAR CAPACITY & PERIMETER EVALUATOR
// ============================================================================
function calculatePunchingCapacity(
  fc: number, 
  c1: number, 
  c2: number, 
  d: number, 
  ...
  switch (code) {
    case 'BS8110': {
      bo = 2 * c1 + 2 * c2 + 8 * (1.5 * d);
      const depthFactor = Math.max(Math.pow(400 / d, 1 / 4), 1.0);
      const vc = (0.79 * Math.pow(100 * 0.005, 1 / 3) * depthFactor) / 1.25;
      phiVc = (vc * bo * d) / 1000;
      clause = 'BS 8110-1 Table 3.8 & Cl 3.7.7';
      break;
    }
    case 'ACI318_19': {
      bo = 2 * (c1 + d) + 2 * (c2 + d);
      phiVc = (0.75 * 0.33 * Math.sqrt(fc) * bo * d) / 1000;
      clause = 'ACI 318-19 Cl. 22.6.5.2';
      break;
    }
    case 'EC2_EN1992': {
      bo = 2 * c1 + 2 * c2 + 2 * Math.PI * (2 * d);
      const k = Math.min(1 + Math.sqrt(200 / d), 2.0);
      phiVc = (0.12 * k * Math.pow(100 * 0.005 * fc, 1 / 3) * bo * d) / 1000;
      clause = 'BS EN 1992-1-1 Cl. 6.4.4';
      break;
    }
    case 'IS456': {
      bo = 2 * (c1 + d) + 2 * (c2 + d);
      phiVc = (1.0 * 0.25 * Math.sqrt(fc) * bo * d) / 1000;
      clause = 'IS 456:2000 Cl. 31.6.3';
      break;
    }
    case 'AS3600': {
      bo = 2 * (c1 + d) + 2 * (c2 + d);
      phiVc = (0.7 * 0.33 * Math.sqrt(fc) * bo * d) / 1000;
      clause = 'AS 3600:2018 Cl. 9.3.1';
      break;
    }
    case 'CSA_A23_3': {
      bo = 2 * (c1 + d) + 2 * (c2 + d);
      phiVc = (0.65 * 0.38 * Math.sqrt(fc) * bo * d) / 1000;
      clause = 'CSA A23.3-19 Cl. 13.3.3';
      break;
    }
    default: {
      bo = 2 * (c1 + d) + 2 * (c2 + d);
...
function generate3DGeometryAndRebars(
  ...
  const footingBoxes: NonNullable<Geometry3DData['footingBoxes']> = [];
  const columnBoxes: NonNullable<Geometry3DData['columnBoxes']> = [];
  ...
  if (input.category === 'shallow') {
    ...
    if (shallowInput.shallowType !== 'combined') {
      ...
      for (let i = 0; i < numBarsZ; i++) {
        const z = -L_m / 2 + cov_m + i * ((L_m - 2 * cov_m) / Math.max(numBarsZ - 1, 1));
        rebars.push({
          mark: `BOT-X-${i + 1}`,
          barDiameter: barDiam,
          diameter: barDiam,
      ...
      for (let i = 0; i < numBarsX; i++) {
        const x = -B_m / 2 + cov_m + i * ((B_m - 2 * cov_m) / Math.max(numBarsX - 1, 1));
        rebars.push({
          mark: `BOT-Z-${i + 1}`,
          barDiameter: barDiam,
          diameter: barDiam,
    ...
    const pDiam_m = (deepInput.pileDiameter ?? 400) / 1000;
    ...
    if (deepInput.deepType === 'drilled_shaft' || deepInput.deepType === 'single_pile') {
      ...
      const s_pile = (deepInput.pileSpacing ?? 3 * (deepInput.pileDiameter ?? 400)) / 1000;
...
function designShallowFoundation(input: ShallowDesignInput): FoundationDesignResult {
  ...
  // Punching Shear Verification & Depth Iteration
  let d = D - cover - 10;
  let shearSafe = false;
  ...
  while (!shearSafe && D <= 2200) {
    ...
    // Critical punching area calculation
    const A_punch = ((c1 + d) / 1000) * ((c2 + d) / 1000);
    const Vu = Math.max(Pu - qu * A_punch, 0);

    const check = calculatePunchingCapacity(fc, c1, c2, d, code);
  ...
  const numBarsB = Math.ceil((L - 2 * cover) / Math.max(botBarSpacing, 50)) + 1;
  const numBarsL = Math.ceil((B - 2 * cover) / Math.max(botBarSpacing, 50)) + 1;
  ...
  return {
    ...
    structuralChecks: {
      ...
      punchingShearDcr,
      flexureDcr: Number((AsReqBot / Math.max(As_prov_val, 1)).toFixed(3)),
...
function designDeepFoundation(input: DeepDesignInput): FoundationDesignResult {
  const { pDead, pLive, mDeadX = 0, fc, fy, c1, c2, cover, deepType, code } = input;
  
  const pileDiam = input.pileDiameter ?? 400;
  const pileCap = input.pileCapacity ?? 500;
  ...
  let numPiles = input.numPiles || Math.ceil(((pDead + pLive) * 1.15) / pileCap);
  numPiles = Math.max(numPiles, deepType === 'single_pile' || deepType === 'drilled_shaft' ? 1 : 2);

  const s_pile = input.pileSpacing || Math.max(3 * pileDiam, 900);
  const edgeDist = Math.max(1.5 * pileDiam, 400);

  let B = 0, L = 0;
  if (numPiles === 1) { 
    B = pileDiam + 2 * edgeDist; 
    L = B; 
  } else if (numPiles === 2) { 
    B = pileDiam + 2 * edgeDist; 
    L = s_pile + pileDiam + 2 * edgeDist; 
  } else {
    const side = Math.ceil(Math.sqrt(numPiles));
    B = (side - 1) * s_pile + pileDiam + 2 * edgeDist;
    L = B;
  }

  const D = Math.max(pileDiam + 300, 600);
  const d = D - cover - 25;
  ...
  return {
    ...
    structuralChecks: {
      bearingOrPileDcr: Number((P_per_pile / Math.max(pileCap, 1)).toFixed(3)),
      wideBeamShearDcr: Number((punchingShearDcr * 0.65).toFixed(3)),
      punchingShearDcr, flexureDcr: Number((AsReqBot / Math.max(As_prov_val, 1)).toFixed(3)),
    ...
    concreteVolumeM3: Number(((B / 1000) * (L / 1000) * (D / 1000)).toFixed(2)),
    status: punchingShearDcr <= 1.0 && P_per_pile <= pileCap ? 'OPTIMIZED' : 'OVERSTRESSED',
  };
}